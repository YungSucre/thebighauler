#!/bin/bash
# BOUCLE NOCTURNE thebighauler — production jusqu'au seuil solde 0.20$.
# Vagues de 25 articles → heroes → build → push → check solde → continue.
set -e
cd /root/thebighauler
...
VAGUE=0
SEUIL=0.20
REPO_URL="https://x-access-token:${GH_TOKEN_GLOBAL}@github.com/YungSucre/thebighauler.git"
...
while true; do
  ...
  timeout 900 python3 produce_pop.py --limit 25 > /tmp/bh_wave.log 2>&1 || true
  N=$(grep -c "✓" /tmp/bh_wave.log || echo 0)
  log "  produits: $N"
  ...
  # si des articles publiés n'ont pas de hero physique, on ne push PAS
...
PYEOF
  ...
  if [ "$NOHERO" != "0" ]; then
    ...
    # ne push pas mais continue la boucle (les heroes se remplissent au fil des vagues)
  fi

  # 3. build + push
  if npm run build > /tmp/bh_build.log 2>&1; then
