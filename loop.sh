...
while true; do
  ...
  systemd-run --unit=pop-prod-loop --collect --property=Restart=no \
    ...
    /usr/bin/python3 /root/thebighauler/produce_pop.py --limit 14 --vertical $VERT \
    > /dev/null 2>&1 || true
  ...
  # 3.5 AUDIT ANTI-DOUBLONS (zéro token) : draft les quasi-doublons de la vague
  timeout 120 python3 /root/audit_dupes.py /root/thebighauler >> $LOG 2>&1 || log "  audit dupes timeout"
  # purge des liens internes vers les articles draftés par l'audit
  timeout 60 python3 /root/fix_internal_links.py >> $LOG 2>&1 || true
